...
class ContextTest < Test::Unit::TestCase
  test "authorizing_role_types_for raises when given action is nil" do
    assert_raises(ArgumentError) { content.role_context.authorizing_role_types_for(nil) }
  end
  
  test "authorizing_role_types_for falls back to permissions from root context" do
  ...
  test "authorizing_role_types_for uses object's permissions if given" do
  ...
  test "expand_roles_for permutate all roles from authorizing_role_types_for w/ all context types" do
    content = self.content
